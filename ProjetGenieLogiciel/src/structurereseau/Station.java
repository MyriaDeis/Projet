 ...
 * en principe termin�
 */
    public static ArrayList<ArrayList<Fragment>> chemin(ArrayList<Fragment> parcours, Station depart, Station but, ArrayList<ArrayList<Fragment>> list) { // parcours : effectu� jusqu'� pr�sent
		if (parcours.size() == 0) { // on commence la recherche
			...
			if (parcours.get(parcours.size()).getArrivee().compareTo(but.name) == 0) { // condition d'arr�t : chemin trouv� entre les 2 stations
